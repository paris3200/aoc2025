...
const parseInput1 = (rawInput: string): ParsedProblems => {
	...
	lines.forEach((input, index) => {
		const normalizedLine = input.replace(/\s+/g, " ").trim();
...
const part1 = (rawInput: string) => {
	...
	const translatedProblems = translateProblems(problems);
	const answers = mathOperands(translatedProblems, operations);
	const grandTotal = answers.reduce((sum, num) => sum + num, 0);
...
const part2 = (rawInput: string) => {
	...
	const answers = mathOperands(problems, operations.reverse());
	// console.log(reduced)
	const grandTotal = answers.reduce((sum, num) => sum + num, 0);
