...
const parseInput = (rawInput: string) => {
	...
	const grid = new Grid(lines.length);
	let start: number[] = [];
	...
	while (points.length > 0) {
		...
		if (point !== undefined) {
			if (grid.has(point[0], point[1] + 1)) {
				const char = grid.get(point[0], point[1] + 1);
				if (char === "^") {
					count += 1;
...
const countPaths = (
	...
	if (grid.has(x, nextY)) {
		const char = grid.get(x, nextY);
		if (char === "^") {
			pathCount =
				countPaths(x - 1, nextY, grid, visited) +
				countPaths(x + 1, nextY, grid, visited);
		} else {
